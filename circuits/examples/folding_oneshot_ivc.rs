//! IVC-flavoured folding example, using the *one-shot* Protogalaxy API
//! ([ProtogalaxyProverOneShot]/[ProtogalaxyVerifierOneShot]) instead of the
//! regular one used by `folding_ivc.rs`.
//!
//! Unlike the regular API (`protogalaxy::{prover, verifier}`), the one-shot
//! prover produces a single, self-contained proof in one `fold` call (no
//! separate `init` + `fold`), and the one-shot verifier's `fold` returns a
//! `CS::VerificationGuard` whose `.verify(...)` performs a genuine end-to-end
//! KZG/pairing check from the proof bytes alone -- it never needs direct
//! access to the prover's internal state (unlike the regular API's `is_sat`,
//! which currently does: see `folding_ivc.rs`).
//!
...
//! instances together using Protogalaxy (one-shot), exactly as `folding.rs`
//! folds copies of `ShaPreImageCircuit`.
//!
...
use midnight_proofs::{
    ...
    poly::{commitment::Guard, kzg::KZGCommitmentScheme, EvaluationDomain},
...
fn main() {
    ...
    // --- 4. Fold the K_PROOFS verifier-circuit instances together, in one shot. ---
    let mut transcript = CircuitTranscript::<PoseidonState<F>>::init();

    let prover_fold = Instant::now();
    ProtogalaxyProverOneShot::<F, KZGCommitmentScheme<E>, { K as usize }>::fold(
        &outer_srs,
        pk.clone(),
        verifier_circuits.clone(),
        1,
        &[
            &[&[], &public_inputs[0]],
            &[&[], &public_inputs[1]],
    ...
    println!(
        "Time for ProtogalaxyProverOneShot::fold: {:?}",
        prover_fold.elapsed()
    );

    // --- 5. Fold and verify on the verifier side. Unlike the regular API's
    // `is_sat`, `.verify(...)` here performs a genuine end-to-end KZG check
    // directly from the proof bytes, with no access to prover-internal state. ---
    ...
    let guard = ProtogalaxyVerifierOneShot::<F, KZGCommitmentScheme<E>, { K as usize }>::fold(
        &vk,
        &[
            &[C::identity()],
            &[C::identity()],
    ...
    println!(
        "Time for ProtogalaxyVerifierOneShot::fold: {:?}",
        verifier_fold.elapsed()
    );

    let verify = Instant::now();
    guard
        .verify(&outer_srs.verifier_params())
        .expect("Verification failed");
    println!("Time for Guard::verify: {:?}", verify.elapsed());

    println!("IVC folding (oneshot) was a success");
    ...
    // the verifier, and confirm it doesn't accept them. ---
    {
        ...
        let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            || -> Result<(), String> {
                ...
                guard.verify(&outer_srs.verifier_params()).map_err(|e| format!("{e:?}"))
            },
        ));
    ...
    // circuits, on the PROVER side, in one shot. The prover dishonestly claims
    // the original (valid) public input for that slot while actually
    // supplying a corrupted inner proof. If the fold itself doesn't already
    // reject this, the resulting proof MUST fail verification. ---
    {
